...
import com.alibaba.middleware.race.mom.util.QueueSizeCounter;
 ...
 */
public class MBBStorage implements Storage {
    ...
    private class InsertWorker extends Thread {
        ...
        private long offset;
        private QueueSizeCounter counter = new QueueSizeCounter();
        ...
        public void run() {
            while (!stop) {
                try {
                    ...
                    counter.put(list.size());

                    int msgSize = 0;
                    for (StorageUnit u : list) {
                        msgSize += u.msg().capacity();
                    }
                    MappedByteBuffer msgBlock = messageChannel.map(FileChannel.MapMode.READ_WRITE, offset, msgSize);
