 ...
 */
public class Improved6DefaultStorage implements Storage {
    ...
    private class InsertTaskParameter {
        ...
        InsertTaskParameter(int size) {
            this.msgBlock = ByteBufferPool.instance().get(size);
            this.msgIds = new ArrayList<MessageId>();
            this.offsetStates = new ArrayList<OffsetState>();
        }

        public void release() {
            ByteBufferPool.instance().release(msgBlock);
        }
    }
